                        ...
                        <form id="update-note-form" action="{{ route('notes.update', $note) }}" method="POST">
                            ...
                            <div class="mt-4">
                                ...
                                <textarea rows="4" name="description"
                                    class="w-full block rounded shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition ease-in-out duration-500">{{ $note->description }}</textarea>
                                @error('description')
                                    <x-alert class="bg-red-100 text-red-700" :message="$message" />
                                @enderror
                            </div>
